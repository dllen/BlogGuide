...
COMMANDS.cat = function(argv, cb) {
    ...
    filenames.forEach(function(filename, i) {
        ...
        else self.document.location.hash = encodeURIComponent("cat " + argv)
...
COMMANDS.cd = function(argv, cb) {
    ...
    else if (entry.type !== 'dir') this._terminal.write('bash: cd: ' + filename + ': Not a directory.');
...
COMMANDS.ls = function(argv, cb) {
    ...
    writeEntry = function(e, str) {
        ...
        if (args.indexOf('l') > -1) {
            ...
            this.write(Array(maxLen - e.name.length + 2).join(' ') + ' ');
        }
    }.bind(this._terminal);
...
COMMANDS.help = function(argv, cb) {
    this._terminal.write('You can view all articles and categories by just <span class="exec">click</span>.<br>' + 'View all categories by command <span class="dir"><a href="javascript:void(0)" onclick="typeCommand(\'ls -l\')">ls -l</a></span><br>' + 'View all categories and articles structure by command <span class="exec"><a href="javascript:void(0)" onclick="typeCommand(\'tree\')">tree</a></span><br>' + 'Clear the terminal by command <span class="img"><a href="javascript:void(0)" onclick="typeCommand(\'clear\')">clear</a></span> or press CTRL+L<br><br>');
    this._terminal.write('All Commands:<br>');
    for (var c in this._terminal.commands) {
        if (this._terminal.commands.hasOwnProperty(c) && !c.startswith('_')) this._terminal.write(c + '  ');
    }
    cb();
}